...
//#line 47 "gramatica.y"
{
                        if(!tablaSimbolo.existeToken(tablaSimbolo.obtenerToken(val_peek(2).ival).getLexema()+'.'+ambitoActual)){ /* chekeamos que no este esa variable redefiniciendose*/
...
//#line 113 "gramatica.y"
{
			 ...
			 if(!tablaSimbolo.existeToken(auxiliar + '.' + ambitoActual)){  /* chekeamos que no este redeclaradose la funcion*/
			    tablaSimbolo.obtenerToken(val_peek(3).ival).setLexema(auxiliar+'.'+ambitoActual);  /* seteamos el nombre , agregandole el ambito al que pertenece*/
			    tablaSimbolo.obtenerToken(val_peek(3).ival).setTipo(tipoActualdeFuncion); /* seteamos el tipo de la funcion*/
			    tablaSimbolo.obtenerToken(val_peek(3).ival).setUso("funcion");
			    tablaSimbolo.obtenerToken(val_peek(3).ival).setTipoParametro(tablaSimbolo.obtenerToken(val_peek(1).ival).getTipo()); /* set al topo del parametro*/
			 ...
			 crearTerceto(new ParserVal(FUNC), val_peek(3), new ParserVal(-1));  /* terceto que indica comienzo de funcion , con su identificador en segunda posicion , el tercer lugar queda nulo*/
 		  }
break;
case 22:
...
//#line 143 "gramatica.y"
{
		...
		yyval.ival=val_peek(0).ival; /* le pasamos al NT(paramentro) el valor de la clave que contiene el token identificador*/
	}
break;
case 26:
...
{                               /* ejemplo = Main.funcion1.funcion2*/
 				...
 				if(tablaSimbolo.obtenerToken(refFuncion).getTipo() != val_peek(2).sval)  /* chekeo que el tipo de funcion sea el mismo que estoy retornando*/
 				...
 				crearTerceto(new ParserVal(RETURN), val_peek(2), new ParserVal(-1));     /* terceto que indica retorno de funcion , con su valor a retornar en la segunda posicion , el tercer lugar queda nulo*/
...
{				/* aux:= expresion;*/
		       ...
		       yyval = new ParserVal((double)crearTerceto(new ParserVal(ASIG), val_peek(3), val_peek(1)));  /* terceto que indica una asignacion (ASIG) , con su identificador a asignar en segunda posicion , y el identificador que se debe asignar en tercera posicion*/
...
//#line 430 "gramatica.y"
{
 			 ...
 			 yyval =  new ParserVal((double)crearTerceto(new ParserVal(DOUBLE), val_peek(1), new ParserVal(-1)));  /* terceto que indica una conversion (DOUBLE) , con su valor a retornar en la segunda posicion , el tercer lugar queda nulo*/
...
//#line 509 "gramatica.y"
{
       ...
       yyval = val_peek(0);   /* le asigno al NT Factor (usando el atributo ival) , la clave en tabla del simbolos que contiene el Token correspondiente al identificador (que posiblemente sea variable o parametro)*/
       yyval.sval=tablaSimbolo.obtenerToken(val_peek(0).ival).getTipo(); /* le asigno al NT Factor (usando el atributo sval) , el valor correspondiente al tipo que posee el identificador*/
     }
break;
case 111:
...
{  yyval = val_peek(0);            	/* le asigno al NT Factor (usando el atributo ival) , la clave en tabla del simbolos que contiene el Token correspondiente a la constante*/
                       yyval.sval="ULONG";}
break;
case 114:
